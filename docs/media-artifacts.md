...
Daily-driver deployments can set `BMD_MEDIA_ROOT` to a WSL-mounted NAS dataset while leaving SQLite-authoritative text, WAL, derivatives, config, state, and service units on the local WSL filesystem. Explicit external roots require mount and identity-marker proof. When enabled, a bounded `BMD_MEDIA_SPOOL_ROOT` beneath the local data root temporarily owns outage bytes; each row records which root owns it. A steady-state worker pass automatically drains at most its batch limit once the guarded root is ready, while the manual dry-run-first drain remains an operator override.
...
- WSL daemon media worker first performs a bounded automatic spool drain when the guarded final root is ready, then backfills public `http:`, `https:`, and `data:` refs through `media_fetch_tasks` leases; manual `/fetch-pending` and `media_fetch_on_capture=True` background fetches use the same lease path rather than bypassing worker ownership.
