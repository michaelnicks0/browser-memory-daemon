...
- `media_fetch.py` owns that guarded HTTP/data boundary; `media_hls.py` owns bounded playlist parsing and assembly and routes every derived request back through the guarded fetch primitive. `media.py` retains compatible facade imports.
