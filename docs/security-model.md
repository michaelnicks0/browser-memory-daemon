...
- Set `BMD_REQUIRE_BLOB_ROOT_MOUNT=1` with NAS/NFS/SSHFS-backed `BMD_BLOB_ROOT` deployments to make the installer and daemon refuse startup when the mount is absent; daily-driver health reports the same mount guard. Without that explicit guard, local daemon startup and text capture do not create or touch blob roots.
...
- Media and legacy-sidecar blob operations are root-scoped through one `BlobStore`: writes use unique streaming stages, optional size/hash checks, file and parent-directory `fsync`, and atomic replace; media filenames use hashed storage stems; and read/serve/stat/purge/forget/migration flows refuse stale or tampered DB locators outside `clean-text/` or `media/`. Media rows dual-write root-relative and absolute compatibility locators. Reads prefer a populated relative locator, use the legacy path only when the relative field is absent, and fail closed rather than downgrading when the preferred locator is invalid.
