...
- Clean-text and media blob operations are root-scoped through one `BlobStore`: writes use unique streaming stages, optional size/hash checks, file and parent-directory `fsync`, and atomic replace; media filenames use hashed storage stems; and read/serve/stat/purge/forget/migration flows refuse stale or tampered DB locators outside `clean-text/` or `media/`. New rows dual-write root-relative and absolute compatibility locators. Reads prefer a populated relative locator, use the legacy path only when the relative field is absent, and do not downgrade after an invalid preferred locator. Calls without runtime configuration cannot fall back to unchecked DB-path access.
