...
- Migration execution is serialized per runtime with a state-root advisory lock so concurrent startup/CLI migrators cannot interleave ledger inspection and schema steps.
...
| `12` | `normalize-historical-media-state` | Perform one-time historical media status/task correction formerly run by every worker pass. Terminal budget skips remain closed until an explicit scoped operator requeue. |
| `13` | `add-media-cache-reservations` | Add expiring SQLite reservations used to serialize committed-plus-reserved snapshot/domain/global media-cache admission across daemon and worker processes. The migration is additive and moves or deletes no bytes. |
