...
| Media artifacts | ✅ Durable image/video refs and binaries: fast text/manifest capture, browser IndexedDB lazy queue, credentialed fetch, raw blob upload, enabled-by-default X/Twitter CDP recorder with disable control, HLS/audio sidecar handling, explicit state taxonomy, atomic task leasing/retry repository, isolated cache admission/eviction store, daemon public worker, rolling cache gates, purge/rehydrate controls; no OCR/media indexing. | `media_models.py`, `media_tasks.py`, `media_store.py`, `media.py` compatibility facade, `media_worker.py`, `media_queue.js`, `cdp_recorder.js`, integration/e2e tests. |
