...
|---|---|---|
| Windows Chrome extension | ✅ MV3 extension with listener-only service-worker orchestration over extracted typed config, capture/lifecycle bridge, credentialed media bridge, redaction-safe telemetry, visit tracking, idempotent active-tab injection, and durable CDP session/recovery/event controllers; full deterministic SHA-256 capture digests; computed-style/ancestor rendered-visibility extraction; opaque observation/navigation IDs; transactional IndexedDB capture/lifecycle outboxes; and a separate version-2 media task/blob queue with atomic compensation batches, count/byte quotas, stale recovery, 24-hour terminal quarantine, and redaction-safe popup/options telemetry. | `extension/src/config_store.js`, `extension/src/capture_bridge.js`, `extension/src/media_bridge.js`, `extension/src/telemetry.js`, `extension/src/visit_tracker.js`, `extension/src/injection.js`, `extension/src/cdp_session.js`, `extension/src/capture_digest.js`, `extension/src/extractor.js`, `extension/src/outbox.js`, `extension/src/media_queue.js`, `extension/src/service_worker.js`, extension unit tests, isolated real Chrome e2e. |
| WSL daemon | ✅ Loopback HTTP daemon with bearer auth, a standard-library transport adapter, explicit request-independent application use cases, and a composition-only `app.py`. | `http_server.py`, `application.py`, `app.py`, application unit tests, HTTP e2e tests, ADR-0056. |
| SQLite/FTS/media storage | ✅ Documents/visits/snapshots/chunks/FTS/media/audit/deletion receipts plus version-4 through version-8 provenance/locator expansion, version-9 complete cleaned-text authority, version-10 media storage tiers/spool reservations, version-11 blob lifecycle records, version-12 historical media-state correction, version-13 transactional cache reservations, and version-14 immutable observation ingest sequences. New text/provenance commits locally without media-root access; recent/timeline/detail reads are observation-first with explicit legacy fallback. | `schema.sql`, `migration_steps/`, `migrations.py`, `text_authority.py`, `media_storage.py`, `blob_lifecycle.py`, migration/text/read tests, ADR-0028, ADR-0030 through ADR-0040, ADR-0046, ADR-0047, ADR-0060. |
...
| CLI | ✅ serve/health/migrate/doctor/daily-driver-health/search/recent/timeline/detail/query-only X observation export/policy/forget/capture-fixture/media-worker/media-cache/media-spool/blob-root migration/snapshot-text reconciliation/storage reconciliation/backup create/restore. | `cli.py`, CLI/export/backup integration tests. |
...
| SQLite can grow over time. | Media blobs have size/cache gates plus purge/rehydrate controls; ADR-0019 keeps text durable by default, while automatic retention, compaction, and backup-pruning remain follow-up work. |
