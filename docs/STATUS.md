...
| Media artifacts | ✅ Durable image/video refs and binaries: fast text/manifest capture, browser IndexedDB lazy queue, credentialed fetch, raw blob upload, enabled-by-default X/Twitter CDP recorder with disable control, HLS/audio sidecar handling, explicit state taxonomy, atomic task leasing/retry repository, artifact publication/read/purge store with failed-write compensation, daemon public worker, rolling cache gates, purge/rehydrate controls; no OCR/media indexing. | `media_models.py`, `media_tasks.py`, `media_store.py`, `media.py`, worker/extension tests, ADR-0042 through ADR-0044. |
