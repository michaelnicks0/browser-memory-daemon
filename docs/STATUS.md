...
| Media artifacts | ✅ Durable image/video refs and binaries: fast text/manifest capture, browser IndexedDB lazy queue, credentialed fetch, raw blob upload, enabled-by-default X/Twitter CDP recorder, guarded public transport, bounded HLS/audio assembly, explicit state/task stores, bounded current-state reconciliation, scoped dry-run-first budget requeue, and artifact publication/read/purge with failed-write compensation; no OCR/media indexing. | `media_models.py`, `media_tasks.py`, `media_store.py`, `media_fetch.py`, `media_hls.py`, `media_ops.py`, worker/extension tests, ADR-0042 through ADR-0046. |
