...
| Media artifacts | ✅ Durable image/video refs and streamed binaries: fast text/manifest capture, browser IndexedDB lazy queue, credentialed raw upload, enabled-by-default X/Twitter CDP recorder, guarded public transport, bounded streamed HLS/audio assembly, process request/byte budgets, transactional cross-process cache reservations, just-in-time task claiming, explicit state/task stores, bounded current-state reconciliation, scoped dry-run-first budget requeue, and failed-write compensation; no OCR/media indexing. | `media_models.py`, `media_tasks.py`, `media_store.py`, `media_fetch.py`, `media_hls.py`, `media_resources.py`, `media_ops.py`, worker/extension tests, ADR-0042 through ADR-0047. |
