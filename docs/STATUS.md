...
| SQLite/FTS/media storage | ✅ Documents/visits/snapshots/chunks/FTS/media/audit/deletion receipts plus version-4 observation/claim tables, version-5 historical observation backfill, version-6 media-observation links, exact schema fingerprint, ordered checksum ledger, and restore-backed migration gate. Ingest dual-writes observations/claims/media provenance; observation-first activity readers remain target state. | `schema.sql`, `migrations.py`, `migration_steps/`, `ingest.py`, integration/e2e tests. |
