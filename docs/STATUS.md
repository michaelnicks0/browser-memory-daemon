...
| SQLite/FTS/media storage | ✅ Documents/visits/snapshots/chunks/FTS/media/audit/deletion receipts plus version-4 through version-8 provenance/locator expansion, version-9 complete cleaned-text authority, version-10 media storage tiers/spool reservations, and version-11 blob lifecycle records. New text/provenance commits locally without media-root access; recent/timeline/detail reads are observation-first with explicit legacy fallback. | `schema.sql`, `migration_steps/`, `migrations.py`, `text_authority.py`, `media_storage.py`, `blob_lifecycle.py`, migration/text/read tests, ADR-0028, ADR-0030 through ADR-0040. |
...
| Media artifacts | ✅ Durable image/video refs and binaries: fast text/manifest capture, browser IndexedDB lazy queue, credentialed fetch, raw blob upload, enabled-by-default X/Twitter CDP recorder with disable control, guarded public transport, bounded HLS/audio assembly, explicit state taxonomy, atomic task leasing/retry repository, artifact publication/read/purge store with failed-write compensation, daemon public worker, rolling cache gates, purge/rehydrate controls; no OCR/media indexing. | `media_models.py`, `media_tasks.py`, `media_store.py`, `media_fetch.py`, `media_hls.py`, `media.py`, worker/extension tests, ADR-0042 through ADR-0045. |
...
The canonical catalog contains **43 stable requirements**: **37 active** and **6 planned**. Normative statements, implementation links, V-model evidence, and legacy aliases are owned by [`requirements/catalog.toml`](../requirements/catalog.toml); generated tables in this doc set must not be hand-edited.
