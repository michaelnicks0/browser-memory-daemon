...
| Media artifacts | ✅ Durable image/video refs and binaries: fast text/manifest capture, browser IndexedDB lazy queue, credentialed fetch, raw blob upload, enabled-by-default X/Twitter CDP recorder with disable control, HLS/audio sidecar handling, explicit state taxonomy, atomic task leasing/retry repository, daemon public worker, rolling cache gates, purge/rehydrate controls; no OCR/media indexing. | `media_models.py`, `media_tasks.py`, `media.py` compatibility facade, `media_worker.py`, `media_queue.js`, `cdp_recorder.js`, `/media-artifacts/*`, ADR-0042, real Chrome e2e. |
