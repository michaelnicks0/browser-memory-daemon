...
3. Enqueue, claim, checkpoint, acknowledge, retry, and stale-claim recovery run in IndexedDB read/write transactions. Claims are token-checked; a different worker instance cannot acknowledge or retry work it does not own.
...
8. Preserve the existing 100-capture and 200-lifecycle item limits for this slice, but reject new work visibly with aggregate count/serialized-byte telemetry. Byte-quota admission, popup/options telemetry, and lifecycle compaction policy remain a subsequent Phase 5 slice.
...
- The current item limits are visible count backpressure, not the final serialized-byte quota policy. `REQ-037` remains planned until byte quotas, operator telemetry, and specialized media queue quota/cleanup behavior close.
