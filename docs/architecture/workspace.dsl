    ...
    model {
        ...
        browserMemoryDaemon = softwareSystem "Browser Memory Daemon" "Local-first personal recall system that captures Windows Chrome page text and media references, stores them in WSL, and exposes exact search, timeline, detail, deletion, diagnostics, and media cache operations." {
            chromeExtension = container "Chrome MV3 Extension" "Captures visible page text, media references, tab lifecycle events, and browser-side media bytes from Windows Chrome; queues work durably and posts to the WSL daemon." "JavaScript, Chrome Manifest V3" {
                manifestEnvelope = component "Manifest and Permission Envelope" "Declares MV3 permissions, host permissions, service worker, popup, and options entrypoints." "manifest.json"
                extractor = component "Extractor" "Traverses visible DOM text and discovers image/video references while applying the selected policy mode." "JavaScript"
                contentScript = component "Content Script" "Schedules initial, delayed, and SPA captures; tracks scroll; sends capture and inline blob upload messages to the service worker." "JavaScript content script"
                serviceWorker = component "Service Worker" "Owns daemon transport, bearer token use, capture and visit queues, lifecycle state, media queue draining, and CDP recorder orchestration." "JavaScript MV3 service worker"
                browserMediaQueue = component "Browser Media Queue Adapter" "Persists media tasks and fetched blobs in IndexedDB so browser-side media fetch/upload can survive MV3 worker suspension." "JavaScript + IndexedDB"
                cdpRecorder = component "CDP Recorder" "Uses chrome.debugger on configured X/Twitter tabs to capture video.twimg.com HLS manifests and media segments before they become opaque blob player URLs." "Chrome DevTools Protocol"
                popupOptions = component "Popup and Options UI" "Lets the operator view status, pause/resume capture, select policy mode, and trigger local controls from the extension." "HTML/JavaScript"
            }
            ...
            wslLoopbackDaemon = container "WSL Loopback HTTP Daemon" "Authenticated loopback HTTP API that handles capture, visit events, media artifact upload/fetch/purge, exact search, recent/timeline/detail, policy rules, doctor, forget, and static UI serving." "Python 3.11, ThreadingHTTPServer" {
                httpRouter = component "HTTP Request Router" "Routes loopback API requests, serves UI assets, enforces bearer auth for memory/admin APIs, and applies CORS for allowed origins." "Python http.server"
                policyEngine = component "Policy Engine" "Evaluates all/recall/balanced/strict capture mode decisions and redacts URL/title/body text outside all mode." "Python"
                policyStore = component "Policy Store" "Persists and evaluates explicit local block-domain and URL-prefix rules for every policy mode." "Python + SQLite"
                ingestPipeline = component "Ingest Pipeline" "Normalizes URLs, computes document/snapshot IDs, stores visits/snapshots/chunks/FTS rows, writes clean text blobs, and records media references." "Python + sqlite3"
                lifecyclePipeline = component "Lifecycle Pipeline" "Stores metadata-only tab lifecycle events and updates visit dwell seconds idempotently." "Python + sqlite3"
                mediaManager = component "Media Artifact Manager" "Records media references, validates blob uploads, enforces MIME/size/cache gates, writes blobs atomically, queues public fetch tasks, and purges or rehydrates cache entries." "Python + sqlite3 + filesystem"
                searchReadModel = component "Search and Read Model" "Provides exact FTS search, recent captures, timeline, document detail, snapshot detail, and media artifact detail views." "Python + SQLite FTS5"
                forgetPipeline = component "Forget Pipeline" "Deletes URL/domain-scoped memory rows, FTS entries, clean-text blobs, media blobs, lifecycle rows, and records deletion receipts." "Python + sqlite3"
                opsDoctor = component "Ops Doctor and Audit" "Reports health, DB integrity, FTS consistency, runtime paths, storage counts, media queue status, and writes metadata-only audit events." "Python + sqlite3"
            }
        ...
        searchReadModel -> cleanTextBlobStore "Reads full snapshot text from" "Filesystem"
        searchReadModel -> mediaBlobCache "Checks and serves media files from" "Filesystem"
        forgetPipeline -> sqliteDatabase "Deletes rows and records receipts in" "sqlite3"
        forgetPipeline -> cleanTextBlobStore "Deletes text blobs from" "Filesystem"
        forgetPipeline -> mediaBlobCache "Deletes media blobs from" "Filesystem"
