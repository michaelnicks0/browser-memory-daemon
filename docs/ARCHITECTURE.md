...
| REQ-037 | 1 | planned | Browser capture, lifecycle, and media queues shall be transactional, restart-safe, quota-aware, and never silently truncate. | Extension Durable Outboxes | `HRD-012` | `extension/src/outbox.js`<br>`extension/src/service_worker.js`<br>`extension/src/media_queue.js` | `extension/tests/unit/service_worker.test.js::service worker preserves queued captures while daemon is down and drains them after reload`<br>`extension/tests/unit/service_worker.test.js::service worker queue overflow preserves old captures and visibly rejects the new capture` |
