...
| REQ-037 | 1 | planned | Browser capture, lifecycle, and media queues shall be transactional, restart-safe, quota-aware, and never silently truncate. | Extension Durable Outboxes | `HRD-012` | `extension/src/outbox.js`<br>`extension/src/service_worker.js`<br>`extension/src/popup.js`<br>`extension/src/options.js`<br>`extension/src/media_queue.js` | `extension/tests/unit/service_worker.test.js::service worker preserves queued captures while daemon is down and drains them after reload`<br>`extension/tests/unit/service_worker.test.js::service worker queue overflow preserves old captures and visibly rejects the new capture`<br>`extension/tests/unit/service_worker.test.js::service worker enforces byte quota and exposes redaction-safe outbox telemetry` |
...
| Capture/lifecycle outbox | Versioned IndexedDB rows with sequence IDs, atomic claims, token-checked checkpoint/ack/retry, stale-claim recovery, bounded drains, legacy-array import, item admission limits, and serialized-byte accounting. | Existing queued rows survive count-cap exhaustion; new work is visibly rejected. Legacy `chrome.storage.local` arrays are imported before deletion and remain only as a one-version failure fallback. Final byte quotas and operator telemetry remain Phase 5 work. |
