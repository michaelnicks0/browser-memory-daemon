...
Media status vocabulary and ordinary transition matrices live in `media_models.py`. Durable task identity, terminal-state preservation, atomic leasing, stale-lease recovery, and retry/backoff live in `media_tasks.py`. Artifact publication, row ownership, contained reads, cache byte accounting/admission, purge, oldest-first eviction, and tombstone-backed deletion outcomes live in `media_store.py`. Each replacement uses a unique staged candidate so a failed row update can remove the candidate without overwriting the previous committed bytes. Guarded HTTP/data transport and redirect/address policy live in `media_fetch.py`; bounded HLS parsing and assembly live in `media_hls.py`, with every child request routed back through the guarded fetch primitive. `media.py` remains the stable compatibility surface. Streamed assembly and global in-flight budgets remain a later reversible slice.
