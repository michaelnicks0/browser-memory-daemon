...
| Cache management | Purge/rehydrate controls plus oldest-first rolling eviction for domain/global caps. | `media_store.py`, `media.py`, `cli.py`, `/media-artifacts/*` |

Media status vocabulary and ordinary transition matrices live in `media_models.py`. Durable task identity, terminal-state preservation, atomic leasing, stale-lease recovery, and retry/backoff live in `media_tasks.py`. Cache byte accounting, admission, oldest-first eviction, and tombstone-backed deletion outcomes live in `media_store.py`. `media.py` remains the stable public compatibility surface while artifact publication/read/purge and fetch/HLS concerns are extracted in later reversible slices; ADR-0042 and ADR-0043 record these boundaries.
