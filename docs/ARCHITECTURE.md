...
| REQ-040 | 1 | planned | HTTP behavior shall provide stable typed errors, request IDs, secure headers, and bounded streaming. | WSL HTTP Transport | `HRD-015` | `daemon/src/browser_memory_daemon/app.py`<br>`daemon/src/browser_memory_daemon/routes.py` | — |
...
| Daemon API | Standard-library HTTP adapter plus immutable method/path route descriptors, auth, CORS, UI serving, token bootstrap, raw media blob upload, and cache controls. | `/health` public loopback; `/ui` HTML gets same-origin token bootstrap; memory APIs tokened; descriptor matching preserves static-before-parameter precedence and existing response contracts. |
