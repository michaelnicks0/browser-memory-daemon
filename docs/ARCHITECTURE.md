...
| REQ-027 | 1 | active | Daemon-public network fetches shall contact only approved destinations and shall revalidate every redirect and HLS-derived request. | Guarded Media Fetch | `HRD-002` | `daemon/src/browser_memory_daemon/media_fetch.py`<br>`daemon/src/browser_memory_daemon/media_hls.py`<br>`daemon/src/browser_memory_daemon/media.py`<br>`daemon/src/browser_memory_daemon/config.py` | `daemon/tests/integration/test_media_worker.py::test_guarded_public_fetch_revalidates_public_to_private_redirect` |
...
| Daemon lazy sidecar | Public unauthenticated backfill with atomic just-in-time leases, backoff, streamed guarded HLS assembly, process resource budgets, and status classification. | `daemon/src/browser_memory_daemon/media_worker.py`, `media_tasks.py`, `media_models.py`, `media_fetch.py`, `media_hls.py`, `media_resources.py`, `media.py` compatibility facade |
