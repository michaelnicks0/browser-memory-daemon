...
| Daemon lazy sidecar | Public unauthenticated backfill with atomic leases, backoff, HLS assembly, and status classification. | `daemon/src/browser_memory_daemon/media_worker.py`, `media_tasks.py`, `media_models.py`, `media.py` compatibility facade |
...
Media status vocabulary and ordinary transition matrices live in `media_models.py`. Durable task identity, terminal-state preservation, atomic leasing, stale-lease recovery, and retry/backoff live in `media_tasks.py`. Artifact publication, row ownership, contained reads, cache byte accounting/admission, purge, oldest-first eviction, and tombstone-backed deletion outcomes live in `media_store.py`. Each replacement uses a unique staged candidate so a failed row update can remove the candidate without overwriting the previous committed bytes. `media.py` remains the stable public compatibility surface and still owns bounded upload buffering plus guarded fetch/HLS transport until later reversible slices.
