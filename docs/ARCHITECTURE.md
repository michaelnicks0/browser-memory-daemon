...
| BlobStore | Contains blob path resolution plus streaming stage, size/hash verification, atomic commit, read/stat, and delete operations. | New rows dual-write root-relative and absolute compatibility locators; reads prefer relative locators and fall back only when the relative field is absent. Durable tombstone/reconciliation states remain a later Phase 3 slice. |
