...
| Service worker | Register MV3 listeners and orchestrate daemon transport, transactional drains, alarms, and extracted browser-state controllers. | Text capture does not wait on media bytes; capture and media bridge code remains here pending later reversible extraction. |
...
| CDP session controller | Reconcile debugger attachment state and recover minimal capture provenance after worker restart. | Accepts attachment recovery only with matching `chrome.debugger.getTargets` evidence; mismatched page URLs clear stale provenance. |
