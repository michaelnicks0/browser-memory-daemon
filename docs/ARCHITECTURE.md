...
| REQ-040 | 1 | planned | HTTP behavior shall provide stable typed errors, request IDs, secure headers, and bounded streaming. | WSL HTTP Transport | `HRD-015` | `daemon/src/browser_memory_daemon/app.py`<br>`daemon/src/browser_memory_daemon/routes.py`<br>`daemon/src/browser_memory_daemon/api_errors.py` | — |
...
| Daemon API | Standard-library HTTP adapter plus immutable method/path route descriptors, typed status/code mapping, auth, CORS, UI serving, token bootstrap, raw media blob upload, and cache controls. | `/health` public loopback; `/ui` HTML gets same-origin token bootstrap; memory APIs tokened; compatible `error` strings gain stable `error_code`; database/resource failures map to 503 and unexpected exception details are sanitized. |
