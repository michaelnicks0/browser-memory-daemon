...
| BlobStore | Contains blob path resolution plus streaming stage, size/hash verification, atomic commit, read/stat, and delete operations. | Current absolute DB locators remain compatible; relative locators and durable tombstone/reconciliation states are later Phase 3 slices. |
