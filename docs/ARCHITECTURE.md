...
| Service worker | Auth, injection, lifecycle state, stable observation/navigation identity, transactional outbox orchestration, browser lazy media queue/drain, daemon POST/PUTs, alarms, and CDP recorder orchestration. | Text capture does not wait on media bytes; decorated queued IDs survive retry/restart and rotate with tab/URL navigation state. |
