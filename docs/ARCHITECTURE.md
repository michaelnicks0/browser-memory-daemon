...
| Capture observation / URL-claim model | Additive temporal observation and untrusted claim authority in SQLite. | Version-4 tables plus version-5 evidence-bounded backfill are current; ingest dual-writes observations/claims, while observation-first read cutover remains target state. |
